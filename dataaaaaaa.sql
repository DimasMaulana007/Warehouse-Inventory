...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`ipak` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci */;
...
CREATE TABLE `assembly_barang_jadi` (
  ...
  CONSTRAINT `assembly_barang_jadi_ibfk_3` FOREIGN KEY (`id_detail_komposisi`) REFERENCES `detail_bahan_utama` (`id_detail_bahan_utama`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

/*Table structure for table `atk_keluar` */

DROP TABLE IF EXISTS `atk_keluar`;

CREATE TABLE `atk_keluar` (
  `id_atk` char(50) NOT NULL,
  `keluar` int(15) DEFAULT NULL,
  `tanggal` date DEFAULT NULL,
  PRIMARY KEY (`id_atk`),
  CONSTRAINT `atk_keluar_ibfk_1` FOREIGN KEY (`id_atk`) REFERENCES `kode_atk` (`kode_atk`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

/*Table structure for table `atk_masuk` */

DROP TABLE IF EXISTS `atk_masuk`;

CREATE TABLE `atk_masuk` (
  `id_atk` char(50) NOT NULL,
  `masuk` int(15) DEFAULT NULL,
  `tanggal` date DEFAULT NULL,
  PRIMARY KEY (`id_atk`),
  CONSTRAINT `atk_masuk_ibfk_1` FOREIGN KEY (`id_atk`) REFERENCES `kode_atk` (`kode_atk`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
/*Table structure for table `bahan_baku_utama` */

DROP TABLE IF EXISTS `bahan_baku_utama`;

CREATE TABLE `bahan_baku_utama` (
  `id_bahan_utama` int(11) NOT NULL,
  `tanggal` date DEFAULT NULL,
  `qty_hasil` char(50) DEFAULT NULL,
  `id_mesin` char(15) DEFAULT NULL,
  `operator` varchar(50) DEFAULT NULL,
  `lokasi_simpan` enum('Mille','SAS') DEFAULT NULL,
  `nama_bahan` int(15) DEFAULT NULL,
  `bahan_keluar` char(50) DEFAULT NULL,
  `kode_bahan` char(15) DEFAULT NULL,
  PRIMARY KEY (`id_bahan_utama`),
  KEY `nama_bahan` (`nama_bahan`),
  KEY `id_mesin` (`id_mesin`),
  KEY `kode_bahan` (`kode_bahan`),
  CONSTRAINT `bahan_baku_utama_ibfk_1` FOREIGN KEY (`nama_bahan`) REFERENCES `detail_bahan_utama` (`id_detail_bahan_utama`),
  CONSTRAINT `bahan_baku_utama_ibfk_2` FOREIGN KEY (`id_mesin`) REFERENCES `mesin` (`code_mesin`),
  CONSTRAINT `bahan_baku_utama_ibfk_3` FOREIGN KEY (`kode_bahan`) REFERENCES `kode_bahan` (`kode_bahan`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `detail_komposisi` (
  ...
  `qty` int(20) DEFAULT NULL,
  `total_berat_komponen` decimal(10,2) DEFAULT NULL,
...
DROP TABLE IF EXISTS `kode_atk`;

CREATE TABLE `kode_atk` (
  `kode_atk` char(50) NOT NULL,
  `nama_barang` varchar(100) DEFAULT NULL,
  `uraian` enum('Atk Kantor','Atk Kebersihan') DEFAULT NULL,
...
CREATE TABLE `kode_kendaraan` (
  ...
  `kunci/peralatan` varchar(50) DEFAULT NULL,
  `kotak_p3k/apar` varchar(50) DEFAULT NULL,
...
CREATE TABLE `surat_jalan` (
  `id_surat` int(15) NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `surat_jalan_ibfk_2` FOREIGN KEY (`id_customer`) REFERENCES `customer` (`id_customer`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `tanggal_komponen` (
  ...
  `tanggal` date DEFAULT NULL,
  PRIMARY KEY (`id_tgl`)
) ENGINE=InnoDB AUTO_INCREMENT=561338649 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
