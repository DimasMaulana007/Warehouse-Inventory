...
if ( $_GET['bulan'] != '' && $_GET['tahun'] != '') {
    ...
    $download = "<a href='page/stok&laporan/export_kirim_barang.php?bulan=$filterBulan&tahun=$filterTahun' class='btn btn-success'>Download</a>";
    $s=mysqli_prepare($koneksi,"SELECT 
        ...
        WHERE MONTH(sj.tanggal)= ? AND YEAR(sj.tanggal)= ? ORDER BY sj.tanggal ASC");
    ...
    $ket.="
            <tr class='even pointer text-center'>
                ...
                <td class=''>{$data['nama_penerima']}</td>
                <td class=''>{$data['kode_kendaraan']}</td>
                <td class=''>{$data['karakter']}-SSN{$data['susun']}-{$data['nama_warna']}</td>
