...
if($simpan)
{
    ...
    header("location:index.php?url=detail_kirim_barang&id=$rand");
		$query=mysqli_query($koneksi,"insert into surat_jalan(id_surat,no_surat,tanggal,customer,kode_kendaraan,sopir,status)
        values('$rand','$no_sj','$tgl','$custom','$polis','$supir','dikirim')");
}
...
while($data_kode=mysqli_fetch_array($kd)){
	//$tes_kode=['kode_bahan'];
	$pilih.="
		<option value='{$data_kode['id_customer']}'>{$data_kode['nama_customer']} | {$data_kode['alamat']} | {$data_kode['telepon']}</option>
	";
}
$isi="
	...
	<form method='post' class='form-horizontal' id='quickForm'>
        ...
        <div class='card-body'>
            ...
            <div class='form-group row'>
                ...
                <div class='col-sm-5'>
                <select class='form-control select2' name='custom' style='width: 100%;' required>
                    <option>--Pilih--</option>
                    $pilih
                </select>
                </div>
            ...
            <div class='form-group row'>
                <label for='inputEmail3' class='col-sm-2 offset-sm-2 col-form-label'>Supir</label>
                <div class='col-sm-3'>
