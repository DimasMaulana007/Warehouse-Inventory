...
if ($masuk) {
    ...
    $cek = mysqli_query($koneksi, "SELECT * FROM barang_jadi WHERE kode_barang = '$kode'");
    
    if (mysqli_num_rows($cek) > 0) {
        $ket = "<div class='alert alert-warning'>Kode sudah digunakan. Silakan gunakan kode lain.</div>";
    } else {
		$query=mysqli_query($koneksi,"insert into barang_jadi(kode_barang,merk,type,susun,karakter,kunci,hanger,warna)values('$kode','$merk','$type','$susun','$karakter','$kunci','$hanger','$warna')");
		if($query)
		{
...
<section class='content-header'>
	...
	<form method='post' class='form-horizontal' id='quickForm'>
        ...
        <a href='index.php?url=barang' class='btn-sm btn-danger float-right'>Keluar</a>
        </div>
        <div class='card-body'>
            ...
            <div class='form-group row'>
                ...
                <div class='col-sm-3'>
                <input type='text' name='karakter' class='form-control' required>
                </div>
            </div>
			<div class='form-group row'>
