...
if($simpan)
{
    ...
    if (mysqli_num_rows($cek) > 0) {
		...
		$query=mysqli_query($koneksi,"insert into komposisi(kode_komposisi,kode_barang_jadi)values('$kode','$barang')");
		if($query)
		{
			header("location:index.php?url=".encrypt_url('komposisi_komponen')."&id=$kode");
		}
		else
		{
...
FROM 
  barang_jadi 
JOIN 
  komposisi ON komposisi.kode_barang = barang_jadi.kode_barang
JOIN
  TYPE ON type.id_type=barang_jadi.type");
while($data=mysqli_fetch_array($s)){
  ...
  $ket.="
		<tr class='even pointer text-center'>
			<td class=''>$no</td>
      <td class=''>{$data['kode_komposisi']}</td>
			<td class=''>{$data['karakter']}</td>
			<td class=''>{$data['nama']}-{$data['hanger']}</td>
			<td class=''>{$data['susun']}</td>
			<td class='project-actions text-center'><a class='btn btn-primary btn-sm' href='index.php?url=".encrypt_url('detail_komposisi')."&id={$data['kode_komposisi']}'><i class='fas fa-folder'></i>Detail</a>
      <a class='btn btn-info btn-sm' href='index.php?url=".encrypt_url('ekomposisi')."&id={$data['kode_komposisi']}'><i class='fas fa-pencil-alt'></i>Edit</a>
      </td>
		</tr>
	";
}
...
<section class='content'>
	...
	<form method='post' class='form-horizontal' id='quickForm'>
        ...
        <h3 class='card-title'>Kode Assembly</h3>
        </div>
        <div class='card-body'>
		<div class='form-group row'>
			...
			<div class='form-group row'>
                ...
                <div class='col-sm-6'>
                <select class='form-control select2' name='barang' style='width: 100%;' required>
                    <option>--Pilih--</option>
                    $pilih
                </select>
                </div>
      ...
      <div class='container-fluid'>
        <div class='row'>
          <div class='col-sm-12'>
            <div class='card'>
              <div class='card-header'>
                <h3 class='card-title'>Data Assembly</h3>
              </div>
              <!-- /.card-header -->
              <div class='card-body'>
                <table id='example1' class='table table-bordered table-striped'>
                  ...
                  <tr class='text-center'>
                    ...
                    <th class='column-title'>Kode Assembly</th>
