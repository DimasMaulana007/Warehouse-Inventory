...
if($delete!=""){
    ...
    header("location:index.php?url=ekomposisi&id=$id");
}
if($simpan)
{
...
while($data=mysqli_fetch_array($s1)){
	$pilih1.="<option value='{$data['kode_barang']}'>{$data['kode_barang']} | {$data['type_nama']}-{$data['hanger']} | {$data['kunci']} | {$data['karakter']}</option>
	";
}
...
while($data=mysqli_fetch_array($s)){
	$ket.="
		<tr class='even pointer text-center'>
			...
			<td class=''><a href='index.php?url=ekomposisi&id=$id&delete={$data['id_detail_komposisi']}'><i class='fa fa-trash'></i></a></td>
		</tr>
	";
}
...
while($data=mysqli_fetch_array($si)){
	$pilih.="
		<option value='{$data['code_komponen']}'>{$data['code_komponen']} | {$data['nama_komponen']}</option>
	";
}
$d=mysqli_fetch_array(mysqli_query($koneksi,"SELECT 
k.*,
bj.*,
t.type AS nama
FROM 
...
JOIN
  barang_jadi bj ON
  bj.kode_barang = k.kode_barang_jadi
JOIN
  TYPE t ON
  t.id_type = bj.type and kode_komposisi='$id'"));
$ip0=$d["kode_komposisi"];
$ip1="{$d['kode_barang_jadi']} | {$d['karakter']} | {$d['nama']}";
$isi="
<section class='content-header'>
    <div class='container-fluid'>
	<h1>Assembly Barang Jadi</h1>
</section>
    <!-- Main content -->
<section class='content'>
	...
	<form method='post' class='form-horizontal' id='quickForm'>
        ...
        <h3 class='card-title'>Edit Assembly Barang Jadi</h3>
        </div>
        <div class='card-body'>
		<div class='form-group row'>
            ...
            <div class='form-group row'>
                ...
                <div class='col-sm-5'>
                <input type='text' name='kode' class='form-control' value='$ip1' required readonly>
                </div>
            </div>
            <div class='form-group row'>
			...
			<div class='form-group row'>
                ...
                <a href='index.php?url=komposisi' class='btn btn-primary'>Selesai</a>
                <a href='index.php?url=komposisi' class='btn btn-danger'>Keluar</a>
                </div>
            </div>	
        </div>
        </div>
      ...
      <div class='container-fluid'>
        <div class='row'>
          <div class='col-sm-12'>
            <div class='card'>
              <div class='card-header'>
                <h3 class='card-title'>Data Komponen Assembly</h3>
              </div>
              <!-- /.card-header -->
              <div class='card-body'>
